...
interface ChildCategoryFormData {
  name: string;
  slug: string;
...
interface ChildCategoryFormPageProps {
  childCategoryId?: string;
}

export function ChildCategoryFormPage({ childCategoryId }: ChildCategoryFormPageProps) {
  const router = useRouter();
  const { toast } = useToast();
  const [loading, setLoading] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>({});

  const [formData, setFormData] = useState<ChildCategoryFormData>({
    name: '',
    slug: '',
    categoryId: null,
    subcategoryId: null,
  ...
  const updateField = (field: keyof ChildCategoryFormData, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value,
    }));
  ...
  const fetchChildCategory = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: 'Error',
          description: error.error || 'Failed to load child category details',
          variant: 'destructive',
        });
      }
    } catch (error) {
      console.error('Failed to fetch child category:', error);
      toast({
        title: 'Error',
        description: 'Failed to load child category details',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    updateField(name as keyof ChildCategoryFormData, processedValue);

    if (name === 'name' && !childCategoryId) {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) newErrors.name = 'Child category name is required';
  ...
  const handleSubmit = async (event?: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const url = childCategoryId ? `/api/admin/child-categories/${childCategoryId}` : '/api/admin/child-categories';
      const method = childCategoryId ? 'PUT' : 'POST';
      ...
      if (response.ok) {
        ...
        toast({
          title: 'Error',
          description: error.details || error.error || 'Failed to save child category',
          variant: 'destructive',
        });
      }
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: 'An error occurred while saving the child category',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  if (loading && childCategoryId) {
    return (
      <div className='flex items-center justify-center min-h-screen'>
        <p className='text-muted-foreground'>Loading child category...</p>
      </div>
    );
  }
      ...
      <div className='max-w-5xl mx-auto space-y-6'>
        <div className='bg-white rounded-lg shadow-sm p-4 md:p-6'>
          <div className='flex items-center gap-4'>
            ...
            <div>
              <h1 className='text-2xl md:text-3xl font-bold text-slate-900'>{childCategoryId ? 'Edit Child Category' : 'Add New Child Category'}</h1>
              <p className='text-sm text-slate-500'>Manage child category information and settings from a single workspace.</p>
            </div>
          </div>
        </div>
          ...
          <section className='space-y-6'>
            <Card className='bg-white border border-slate-200'>
              <div className='space-y-6 px-6 py-6'>
                <div className='space-y-4'>
                  ...
                  <p className='text-sm text-slate-500'>Provide the child category details.</p>
                  ...
                  <div className='space-y-2'>
                    ...
                    <Select
                      ...
                      onValueChange={val => {
                        updateField('categoryId', val === 'none' ? null : val);
                  ...
                  <div className='space-y-2'>
                    <label className='text-sm font-medium text-gray-700'>
                      Subcategory <span className='text-black'>*</span>
                    </label>
                    <Select
                      value={formData.subcategoryId || 'none'}
                      onValueChange={val => updateField('subcategoryId', val === 'none' ? null : val)}
                      disabled={!formData.categoryId || loadingCategories || loading}>
                      <SelectTrigger className='h-[48px]'>
                ...
                <div className='space-y-4'>
                  ...
                  <p className='text-sm text-slate-500'>Upload images for the child category.</p>
                ...
                <div className='space-y-4'>
                  ...
                  <p className='text-sm text-slate-500'>Configure child category display and ordering.</p>
                  ...
                  <div className='flex items-center justify-between p-4 border rounded-lg'>
                    <div>
                      <p className='text-sm font-medium'>Child Category Status</p>
                      <p className='text-xs text-muted-foreground'>Inactive child categories won't be displayed</p>
                    </div>
                    <Switch
                      id='status'
            ...
            <div className='flex flex-col sm:flex-row gap-3 justify-end pt-4'>
              ...
              <Button type='submit' disabled={loading}>
                {loading ? 'Saving...' : childCategoryId ? 'Update Child Category' : 'Create Child Category'}
              </Button>
            </div>
          </section>
        </form>
