...
export function QualityBannerList() {
  const router = useRouter();
  const { toast } = useToast();
  const [banners, setBanners] = useState<QualityBanner[]>([]);
  ...
  const fetchBanners = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/cms/quality-banners?${params}`);
      if (response.ok) {
      ...
      console.error('[v0] Failed to fetch quality banners:', error);
      setBanners([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/cms/quality-banners/${deleteId}`, {
        method: 'DELETE',
      });
      ...
      if (response.ok) {
        toast({
          title: 'Success',
          description: 'Quality banner deleted successfully',
          variant: 'success',
        });
        fetchBanners();
      } else {
        toast({
          title: 'Error',
          description: 'Failed to delete quality banner',
          variant: 'destructive',
        });
      }
    } catch (error) {
      toast({
        title: 'Error',
        description: 'An error occurred while deleting the quality banner',
        variant: 'destructive',
      });
    } finally {
      setDeleteId(null);
  ...
  const handleToggleStatus = async (bannerId: string, currentStatus: string) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: 'Success',
          description: `Quality banner ${newStatus === 'active' ? 'activated' : 'deactivated'} successfully`,
          variant: 'success',
        });
        fetchBanners();
      } else {
        toast({
          title: 'Error',
          description: 'Failed to update quality banner status',
          variant: 'destructive',
        });
      }
    } catch (error) {
          ...
          <Table>
            <TableHeader>
              <TableRow className='bg-gray-50 border-b border-gray-200 hover:bg-gray-50'>
                ...
                <TableHead className='font-semibold text-gray-700 py-4'>Features</TableHead>
            ...
            <DataTableBody
              ...
              data={banners}
              columns={5}
              loadingText='Loading quality banners...'
              emptyText='No quality banners found'>
              {banners.map(banner => (
                <TableRow key={banner._id} className='border-b border-gray-200 hover:bg-green-50 transition-colors duration-150'>
                  ...
                  <TableCell className='text-sm text-gray-600 py-4'>{banner.features?.length || 0} features</TableCell>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This action cannot be undone. This will permanently delete the quality banner.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
